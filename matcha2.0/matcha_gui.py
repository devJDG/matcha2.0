...
try:
    ...
    messagebox.showerror("Import Error", "Could not find matcha.py. Make sure it's in the same directory.")
    exit()
except Exception as e:
    messagebox.showerror("Import Error", f"An error occurred importing matcha.py:\n{e}")
    exit()
...
class PdfComparatorApp:
    def __init__(self, root):
        ...
        ttk.Label(main_frame, text="Output Dir:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.compare_button = ttk.Button(main_frame, text="Compare PDFs", command=self.start_comparison_thread)
        self.compare_button.grid(row=3, column=0, columnspan=3, pady=15)

        self.status_label = ttk.Label(main_frame, text="Status: Ready", anchor=tk.W, wraplength=550) 
        self.status_label.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), padx=5, pady=5)
    ...
    def select_output_dir(self):
        ...
        if dir_path:
            ...
            self.update_status("Selected Output Directory.")
    ...
    def start_comparison_thread(self):
        ...
        self.update_status("Starting comparison... This may take a moment.")
    ...
    def run_comparison_worker(self, old_pdf, new_pdf, output_dir):
        ...
        try:
            ...
            self.root.after(0, self.comparison_finished, f"Comparison finished successfully in {duration}. Output saved to '{output_dir}'")

        except Exception as e:
            tb_str = traceback.format_exc()
            error_message = f"An error occurred during comparison: {e}\n\n{tb_str}"
            print(error_message) 
            self.root.after(0, self.comparison_failed, f"Error during comparison: {e}")

    def comparison_finished(self, message):
        self.update_status(message)
        self.set_ui_state(True) 
        messagebox.showinfo("Success", "PDF comparison completed successfully!")
